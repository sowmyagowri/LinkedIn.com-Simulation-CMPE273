...
const { SEARCH_PEOPLE_REQUEST, SEARCH_PEOPLE_RESPONSE } = require('./../kafka/topics');
...
router.get("/", (req, res) => {
    ...
    if (errors) {
        ...
        kafka.make_request(SEARCH_PEOPLE_REQUEST, SEARCH_PEOPLE_RESPONSE, req.query, function (err, result) {
            if (err) {
