...
const { CONNECTION_RESPONSE_REQUEST, CONNECTION_RESPONSE_RESPONSE, } = require('./../kafka/topics');
...
router.post("/", (req, res) => {
    ...
    if (errors) {
        ...
        kafka.make_request(CONNECTION_RESPONSE_REQUEST, CONNECTION_RESPONSE_RESPONSE, req.body, function (err, result) {
            if (err) {
...
function validateInputForAddingMessage(req) {
    ...
    req.checkBody("receiver.username", "receiver's firstname is required").notEmpty();
    req.checkBody("sender.username", "sender firstname is required").notEmpty();

    req.checkBody("receiver.firstname", "receiver's firstname is required").notEmpty();
    req.checkBody("sender.firstname", "sender firstname is required").notEmpty();
   
    req.checkBody("receiver.lastname", "receiver's lastname is required").notEmpty();
