...
async function handle_request(req, callback) {
    ...
    try {
        receiverIncomingRequest = await Users.findOneAndUpdate({email:req.receiver.username}, {
            $push: {
                connectionsIncoming: {
        ...
        senderOutgoingRequest = await Users.findOneAndUpdate({email:req.sender.username}, {
            $push: {
                connectionsOutgoing: {
